<template>
  <div>
    ...
    >
      <template v-slot:body-cell-action="texnikalar">
        <q-td :texnikalar="texnikalar" align="left" width="20px">
          <q-btn
            color="blue-8"
            icon="edit"
            v-if="ruxsatlar.edit"
            @click="ShowEditModal(texnikalar.row)"
            size="sm"
            danse
            class="q-ml-sm"
          />
          <q-btn
            ...
            @click="ShowDeleteModal(texnikalar.row)"
            size="sm"
            danse
            class="q-ml-sm"
          />
  ...
  methods: {
    async getData(pagination, search) {
      ...
      await this.$axios
        .post("texnika", data)
        .then((response) => {
          let data = response.data.texnikalar.data;
          this.texnikalar = [];
          let ind =
            (response.data.texnikalar.current_page - 1) *
              response.data.texnikalar.per_page +
            1;
          ...
          this.pagination.rowsNumber = response.data.texnikalar.total;
        })
        .catch((error) => {
    ...
    ShowDeleteModal(data) {
      this.$q
        ...
        .onOk(() => {
          this.$axios
            .post("texnika/delete", { id: data.id })
            .then((response) => {
